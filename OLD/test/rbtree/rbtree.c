...
/*
  The recursive part of the insertion routine.
  ...
  Three cases of red violation:

  CASE 1
  Check both children of a node. If both children are red, then the
  parent has to be black, so flip the colors. The parent becomes red
  and the children become black:

             2,B                          2,R
            /   \                        /   \
          1,R  3,R       BECOMES       1,B   3,B
          /                            /
        0,R                          0,R


  CASE 2
  If the node's sibling is not red. In the diagram below, the left
  child of node 1 is causing the violation, so the tree is single-
  rotated around node 2 to the right. 2 is made red, 1 is made black,
  and the tree is again in conformance.

             2,B                          1,B
            /   \                        /   \
          1,R  3,B       BECOMES       0,R   2,R
          /                                    \
        0,R                                    3,B

  Why is this still an invalid red-black tree?
*/
